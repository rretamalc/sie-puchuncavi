...
class Application_Model_DbTable_Notas extends Zend_Db_Table_Abstract
{
    ...
    public function getnotaspresegmento($id, $idperiodo, $segmento, $idasignatura, $idcurso)
    {
        ...
        $select->join(array('as' => 'asignaturas'), 'as.idAsignatura = asc.idAsignatura', array('idAsignatura', 'nombreAsignatura'));
        $select->joinLeft(array('n' => 'nucleos'), 'n.idNucleo = asc.idNucleo');
        $select->joinLeft(array('am' => 'ambitos'), 'am.idAmbito = n.idAmbito');
    ...
    public function getnotaschocota($curso, $periodo)
    {

        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from(array('n' => 'notas'), array(
            'idNotas',
            'nota',
            'idPruebas',
            'idAlumnos',
        ...
        $select->joinLeft(array('ev' => 'evaluaciones'),
            'ev.idEvaluacion = n.idPruebas', array('idEvaluacion', 'tipoEvaluacion'));

        $select->where('n.idCursos = ?', $curso);
        $select->where('n.idPeriodo = ?', $periodo);

        return $this->fetchAll($select)->toArray();

    }

    public function getnotasguiaschocota($curso, $periodo, $idAsignatura, $tiempoNota, $tipoGuia)
    {

        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from(array('ng' => 'notasGuia'), array(
            'idNotasGuia',
            'nota_1',
            'idGuia',
            'idAlumnos',
        ...
        $select->joinLeft(array('ge' => 'guiasevaluacion'),
            'ge.idGuia = ng.idGuia'
            , array('nombreguia'
                , 'fechaGuia'
                , 'idCuenta'
        ...
        $select->where('ng.idAsignatura = ?', $idAsignatura);
        $select->where('ng.idCursos = ?', $curso);
        $select->where('ng.idPeriodo = ?', $periodo);

        if ($tipoGuia === 'Formativa') {
            $select->where('ge.tipoGuia = ?', 2);
            $select->where('ge.tiempoGuia = ?', $tiempoNota);
        }
    ...
    public function getnotaalumnochocota($idAlumnos, $idEvaluacion)
    {

        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from(array('n' => 'notas'), array('idNotas'));
        $select->where('n.idAlumnos = ?', $idAlumnos);
        $select->where('n.idEvaluacion = ?', $idEvaluacion);

        return $this->fetchAll($select)->toArray();

    }

    public function updatenotachocota($idNota, $nota)
    {
        $data = array(
            'nota' => $nota,
            'valorGuia' => $nota,
        );
        $this->update($data, 'idNotas = ' . (int)$idNota);
    }

    public function updatenotaguiachocota($idNotasGuia, $nota_1)
    {
        $db = new Zend_Db_Table('notasGuia');
        $data = array(
            'nota_1' => $nota_1,
        );
        $db->update($data, 'idNotasGuia = ' . (int)$idNotasGuia);
    }


    public function getnotasguiaalumnochocota($idEvaluacion, $idAlumnos, $tipoGuia) {
        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from(array('ng' => 'notasGuia'), array(
            'nota_1',
        ...
        $select->joinLeft(array('ge' => 'guiasevaluacion'),
            'ge.idGuia = ng.idGuia', array('porcentajeGuia', 'tipoGuia'));

        $select->where('ge.idEvaluacionGuia = ?', $idEvaluacion);
        $select->where('ng.idAlumnos = ?', $idAlumnos);
        if ($tipoGuia === 'Formativa') {
            $select->where('ge.tipoGuia = ?', 2);
        }
    ...
    public function getidnotasevaluacionchocota($idEvaluacion, $idAlumnos) {
        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from(array('n' => 'notas'), array(
            'idNotas',
        ));
        $select->where('n.idPruebas = ?', $idEvaluacion);
        $select->where('n.idAlumnos = ?', $idAlumnos);

        return $this->fetchAll($select)->toArray();
    }

    public function getnotastrimestrechocota($idNota, $idNotaGuia) {
        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from(array('ng' => 'notasGuia'), array(
            'nota_1',
        ...
        $select->where('ng.idNotasGuia != ?', $idNotaGuia);
        $select->where('ng.idNota = ?', $idNota);


        return $this->fetchAll($select)->toArray();
    }

    public function getnotasformativaschocota($curso, $periodo)
    {
        ...
        $select->joinLeft(array('n' => 'notas'),
            'ng.idNota = n.idNotas', array('idPruebas', 'idPruebas', 'idAsignatura', 'nota'));
        $select->joinLeft(array('ev' => 'evaluaciones'),
            'ev.idEvaluacion = n.idPruebas', array('idEvaluacion'));

        $select->where('ng.idCursos = ?', $curso);
        $select->where('ng.idPeriodo = ?', $periodo);
        $select->where('ev.tipoEvaluacion = 2');
        $select->where('ev.tipoEvaluacion IS NOT NULL');

        return $this->fetchAll($select)->toArray();

    }

    public function addnotasguia(
        $nota,
        $fecha,
        $idPeriodo,
        $idAlumno,
    ...
    public function getnota($idEvaluacion, $idAlumno) {
        $select = $this->select();
        $select->setIntegrityCheck(false);
        $select->from(array('notas'),  array(
            'idNotas',
        ));
        $select->where('idPruebas = ?', $idEvaluacion);
        $select->where('idAlumnos = ?', $idAlumno);
        $stmt = $select->query();
        $result = $stmt->fetchAll();
    ...
    {
        //$this->delete borrar album donde RBD=$rbd
        $this->delete('idNotas =' . (int)$idNota);
    }
