			...
			this.evaluaciones.forEach(guia => {
				notasAlumno.forEach(nota => {
					if (guia.idGuia === nota.idGuia) {
						...
						if (nota.tipoGuia === '1' || nota.tipoGuia === '3') {
							const notaDecimal = this.soloNumeros(nota.nota_1)
							notaPorcentaje = (Number(notaDecimal) * Number(guia.porcentaje)) / 100
							...
							if (sumaPromedio !== 0) {
								...
								notaPorcentaje = (Number(notaDecimal) * Number(nota.porcentajeGuia)) / 100
							}
						}
						promedioNota = Number(promedioNota) + Number(notaPorcentaje)

						console.log('promedioNota :', promedioNota)

					}
				})
			})
			this.promedio = this.soloNumeros(Number.parseFloat(promedioNota))
