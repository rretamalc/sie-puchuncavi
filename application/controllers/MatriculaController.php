...
//        $this->view->title = "Matricular Alumno";
...
//                    }
//
//                    //creo objeto alumnosque controla la tabla Alumnos de la base de datos
...
//
//                        $alumnos->agregar($rut, $nombres, $apaterno, $amaterno, $direccion, $telefono, $movil, $comuna, $correo, $sexo, $fechanacimiento, $idApoderado, $curso, $idApoderados, $prioritario, $beneficio, $numeromatricula, $newFilename);
//                        $idingresado = $alumnos->getAdapter()->lastInsertId();
//                        //creo objeto medicos que controla la talba Alumnos_detalle_clinico de la base de datos
//                        $medicos = new Application_Model_DbTable_Alumnomedico();
//                        $medicos->agregar($prevision, $letra, $actividad, $patologia, $profesional, $tprofesional, $hora, $documento, $idingresado);
//
//                        //creo objeto familiar que controla la talba Alumnos_detalle_nucleo de la base de datos
//                        $familiar = new Application_Model_DbTable_Alumnonucleo();
//                        $familiar->agregar($nombremadre, $apellidomadre, $telefonomadre, $nivelmadre, $nombrepadre, $apellidopadre, $telefonopadre, $nivelpadre, $rutpadre, $rutmadre, $ocupacionpadre, $ocupacionmadre, $idingresado);
...
//
//                        $alumnosactual->agregar($idingresado, $curso, $periodo_get);
//
...
//
//                        $listapruebas = $modelevaluacion->listapruebas($curso, $periodo_get);
//                        $date = new DateTime;
//                        $fechaactual = $date->format('Y-m-d');
//                        for ($i = 0; $i < count($listapruebas); $i++) {
//                            $modelnotas->agregar($idingresado, $listapruebas[$i]['idAsignatura'], $curso, 0, $listapruebas[$i]['idCuenta'], $listapruebas[$i]['idEvaluacion'], $fechaactual, $periodo_get);
//
//                        }
//                        if ($upload) {
//                            $db->commit();
...
//                } else {
//
//                    $messages = $this->_helper->getHelper('FlashMessenger')->addMessage('El Rut de Alumno que intenta ingresar, esta ya ingresado');
//                    /// Assign the messages
//                    $this->view->assign('messages', $messages);
//
//                }
...
//                $rowsetcursos = $cursos->getValueCurso($RBD_alumno);
//
//                $form->idCursos->clearMultiOptions();
//                $form->idCursos->addMultiOptions($rowsetcursos);
//                $form->idCursos->setValue($curso);
//                $form->idEstablecimiento->setValue($RBD_alumno);
