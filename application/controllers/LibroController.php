...
//            //Buscamos si es Prebasica
//            $datospre = $table->listarcursodocentepre($user, $idperiodo, $establecimiento)->toArray();
//
//            if (count($datospre) > 0) {
//                $datoscurso[] = $datospre[0];
//
//            }
    ...
    public function abrirAction()
    {
        ...
        // Si el Rol es Docente y el ingreso de notas es por asignatura
        if ($rol == '2') {
            ...
            if ($id > 0) {
                ...
                //valida que el curso exista para el usuario
                $datosusuario = $modelocurso->getcursohorario($idcuenta, $idperiodo, $id, $idestablecimiento);

                //Valida si el curso es de prebasica y correspnde al usuario
                ...
                //creamos un mensaje en el view
                $messages = $this->_helper->getHelper('FlashMessenger')->addMessage('No se encontro el libro que desea abrir');
                $this->view->assign('messages', $messages);

            }

        } //fin if rol == 2 Profesor
        ...
        if ($rol == '2') {
            ...
            if ($this->getRequest()->isPost()) {
                ...
                if ($formpruebas->isValid($formData)) {
                    ...
                    //creo objeto Establecimiento que controla la talba establecimiento de la base de datos
                    $pruebas = new Application_Model_DbTable_Pruebas();
                    $fecha2 = date("Y-m-d", strtotime($fecha));
                    //cambiamos el formato en el arreglo
                    $fecha = $fecha2;
    ...
    public function pruebaeditarAction()
    {
        ...
        if ($id > 0) {
            ...
            $pruebas = new Application_Model_DbTable_Pruebas();
            //extraigo de la talba el rut Rut= $rut
        ...
        if ($this->getRequest()->isPost()) {
            ...
            //id del periodo que deseo editar
            //si vienbe un parametro llamado id le asigno su valor a $idPeriodo;
    ...
    public function pruebaeliminarAction()
    {
        ...
        //debe venir un parametro, por GET o POST, llamado id, con el id del Establecimiento a borrar
        $id = $this->_getParam('id', 0);
        //creo objeto tabla Establecimiento
        ...
        try {
            ...
            //al listado de Establecimientos
            $this->_helper->redirector('pruebas');
        } catch (Exception $e) {
            // Si hubo problemas. Enviamos todo marcha atras
            $db->rollBack();
    ...
    public function editacomunicacionesAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($id > 0) {
                ...
                //populate() toma los datos de $rut y los coloca en el formualrio.
...
//
//                $datosdocente = $modelocurso->getcursocuentaperiodo($user, $idperiodo);
//                $resultado = array_search($idcurso, array_column($datosdocente, 'idCursos'));
//
//                //Si el Curso Existe buscamos los datos del alumno
...
//            } else {
    ...
    public function guardanotaseditarnuevoAction()
    {
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            //creamos una variable con la fecha actual $fecha
            $date = new DateTime;
            $fecha = $date->format('Y-m-d H:i:s');
...
//            $periodo = new Zend_Session_Namespace('periodo');
//            $idperiodo = $periodo->periodo;
//            //extraemos el establecimiento del usuario que ingresa
//
//            //extreamos el rut del usuario que ingresa
//            $rutusuario = new Zend_Session_Namespace('id');
//            $usuario = $rutusuario->id;
...
//
//                        //$asistencia->agregar($data['0']['curso'], $data[$i]['alumno'], $fecha2, $idperiodo, $usuario, $data[$i]['ausencia']);
//
//                        $asistencia->agregarvalores($data[0]['valores'][$i]['ausencia'], $data[0]['valores'][$i]['alumno'], $idasistencia);
//                    }
//                    $db->commit();
//                    echo Zend_Json::encode(array('redirect' => '/Libro/asistenciames'));
//                } catch (Exception $e) {
//                    echo $e;
//                    // Si hubo problemas. Enviamos  marcha atras
//                    //$db->rollBack();
...
//
//            $modeloasistencia = new Application_Model_DbTable_Asistencia();
//            $datosasistencia = $modeloasistencia->getasistenciaid($idasistencia);
//            $mes = date("n", strtotime($datosasistencia[0]['fechaAsistencia']));
//            $modeloalumnos = new Application_Model_DbTable_Alumnosactual();
//            $listaalumnos = $modeloalumnos->listaralumnoscursoactual($id_curso, $idperiodo);
//
//            //extreamos el rut del usuario que ingresa
...
//                            $modeloasistencia->cambiar($data['asistencia'][$i]['idAsistenciaValores'], $data['asistencia'][$i]['valor']);
//                        }
//                        $db->commit();
//                        $pag = $this->_request->getBaseUrl() . ('/Libro/asistenciames');
//                        die(json_encode(["response" => 1, "url" => $pag, "m" => $mes]));
//                    } catch (Exception $e) {
//                        // Si hubo problemas. Enviamos  marcha atras
//                        $db->rollBack();
...
//        }
...

    {
        ...
        //Zend_Debug::dump($resultadoasignatura);
        ...
        } elseif ($resultadoasignatura[0]['tipoAsignatura'] == 1 || $resultadoasignatura[0]['tipoAsignatura'] == 3 || $resultadoasignatura[0]['tipoAsignatura'] == 4 || $resultadoasignatura[0]['tipoAsignatura'] == 5) {
            for ($i = 0; $i < count($resultado['notas']); $i++) {
                $resultado['notas'][$i]['fechaEvaluacion'] = $this->datetranslate($resultado['notas'][$i]['fechaEvaluacion']);
                $resultado['notas'][$i]['alumnos'] = $modelonotas->getnotasasignatura($resultado['notas'][$i]['idEvaluacion'], $idcurso, $idperiodo);
                //si el resultado de las notas no coinciden con la cantidad de alumnos se realiza una validacion de las notas de esa evaluacion
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            if (empty($usuario)) {
                ...
                try {
                    ...
                    /* RQM 215 */
                    $diasTrabajados = $data[0]['diast'] == "" ? 0 : $data[0]['diast'];
                    $diasInasistencia = $data[0]['diasi'] == "" ? 0 : $data[0]['diasi'];
                    /* FIN RQM 215 */
                    $validadetalle = $modeloalumnodetalle->get($detallealumnoactual[0]['idAlumnosActual'], $data[0]['s'], $idperiodo);
                    if ($validadetalle) {
                        /* RQM 215 */
                        $modeloalumnodetalle->cambiar($diasTrabajados, $diasInasistencia, null, 0, $data[0]['obs'], 0, $data[0]['s'], $validadetalle[0]["idDetalle"]);
                        /* FIN RQM 215 */
                    } else {
                        /* RQM 215 */
                        $modeloalumnodetalle->agregar($diasTrabajados, $diasInasistencia, null, 0, $data[0]['obs'], 0, $data[0]['s'], $detallealumnoactual[0]['idAlumnosActual'], $idperiodo);
                        /* FIN RQM 215 */
                    }
                    ...
                    if ($largoalumnos > 0) {
                        ...
                        for ($i = 0; $i < count($data); $i++) {
                            /* RQM 215 */
                            $pruebas->agregar('', Null, $idcurso, $data[$i]['asignatura'], $idperiodo, $usuario, $data[$i]['s'], 1, 1, null, null, 0, null);
                            /* FIN RQM 215 */
                            $idprueba = $pruebas->getAdapter()->lastInsertId();
    ...
    public function nuevaobservacionAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($formData)) {
                ...
                if ($idalumnos[0] == "allc") {
                    //Obtenemos la lista completa del curso
                    $modeloalumnos = new Application_Model_DbTable_Alumnosactual();
                    $listaalumnos = $modeloalumnos->listaralumnoscursoactual($id_curso, $idperiodo);
                    for ($i = 0; $i < count($listaalumnos); $i++) {
        ...
        if ($activarapp == 1) {
            ...
            if ($rol == '2') {
                ...
                if ($ingresonota == 1) {
                    ...
                    //$this->view->pruebas = $listapruebas;
        ...
        if ($resultadoasignatura[0]['tipoAsignatura'] == 1 || $resultadoasignatura[0]['tipoAsignatura'] == 4 || $resultadoasignatura[0]['tipoAsignatura'] == 5) {
            ...
            try {
                ...
                for ($i = 0; $i < $largoresultado; $i++) {
                    ...
                    // Zend_Debug::dump($resultado['notas'][$i]['alumnos']);
                    // die();
                    $date = new DateTime;
                    $fechacreacion = $date->format('Y-m-d H:i:s');
...
//                            //Agregamos NotasGuia
//
//                            $modelonotas->agregarnotalag(0, 0, 0, $fechacreacion, $idperiodo, $listadealumnos[$j]['idAlumnos'], $resultado['notas'][$i]['idCuenta'], $idcurso, $idasignatura, $resultado['notas'][$i]['idGuia']);
                    ...
                    // Zend_Debug::dump($resultado['notas'][$i]['alumnos']);
                    // Zend_Debug::dump($listadealumnos);
                    // die();

                    // SOP 18 - Laguna
                    foreach ($resultado['notas'][$i]['alumnos'] as $key => $alumno) {
                        if ($alumno['idPeriodo'] == Null
                            && $alumno['idPeriodo'] == Null
                            && $alumno['idAlumnos'] == Null
                            && $alumno['idCuenta'] == Null
                            && $alumno['idCursos'] == Null
                            && $alumno['idAsignatura'] == Null) {
                            //Agregamos la Nota final

                            foreach ($listadealumnos as $key => $listaalumno) {
                                if (
                                    $listaalumno['nombres'] == $alumno['nombres']
                                    && $listaalumno['apaterno'] == $alumno['apaterno']
                                    && $listaalumno['amaterno'] == $alumno['amaterno']
                                    ) {
                                        // Zend_Debug::dump($alumno['idNotas']);
                                        // die();
                                        $modelonotas->borrarNota($alumno['idNotas']);
                                        $modelonotas->agregarnotaguia($listaalumno['idAlumnos'], $idasignatura, $idcurso, 0, $resultado['notas'][$i]['idCuenta'], $resultado['notas'][$i]['idEvaluacion'], $fechacreacion, $idperiodo, 1, 0);
                                        $idnota = $modelonotas->getAdapter()->lastInsertId();

                                        //Agregamos la notaGuia
                                        $modelonotas->agregarnotaguialag($listaalumno['idAlumnos'], $idasignatura, $idcurso, $resultado['notas'][$i]['idCuenta'], $fechacreacion, $idperiodo, 0, 0, 0, $resultado['notas'][$i]['idGuia'], $idnota);
...
//
//                    for ($i = 0; $i < count($resultado); $i++) {
//                        if ($resultado[$i]['concepto'] == trim($data['valor'])) {
//                            $data['valor'] = $resultado[$i]['notaconcepto'];
//                        }
//                    }
//
//
                ...
                if (intval($data['valor']) || $data['valor'] == 0) {
                    ...
                    if ($data['valor'] >= 0 && $data['valor'] <= 100) {
                        ...
                        try {
                            ...
                            //Obtenemos la nota de acuerdo al porcentaje ingresado
                            $modeloequivalencia = new Application_Model_DbTable_Equivalencia();
            ...
            if (empty($usuario)) {
                ...
                if (is_numeric($data['valor_f'])) {
                    ...
                    try {
                        ...
                        // Guardar los valores de la guia
...
//                                $idprueba = $pruebas->getAdapter()->lastInsertId();
...

    {
        ...
        try {
            ...
            //Asignamos la Nota Padre del Primer Semestre
            ...
            for ($i = 0; $i < $largoresultado; $i++) {
                ...
                for ($j = 0; $j < count($listadealumnos); $j++) {
                    ...
                    if(is_null($resultado['notas'][$i]['alumnos'][$j]['idNota'])){
                        //Zend_Debug::dump($resultado['notas'][$i]['alumnos'][$j]['idNotas']);
    ...
    public function controlasistenciasAction()
    {
        ...
        if (!empty($results[0])) {
            ...
            if ($rol == '2') {
                ...
                //$datoscontenido = $modeloasistencia->listarasistencias($id_curso, $user, $idperiodo);
                for ($i = 0; $i < count($datoscontenido); $i++) {
                    //$datoscontenido[$i]['bloques'] = $modeloasistencia->listarbloquesasistencia($datoscontenido[$i]['idControlContenido'],$user);
