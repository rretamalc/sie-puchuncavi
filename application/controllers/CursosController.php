...
////        $form->submit->setLabel('Agregar Curso');
////
////        $this->view->form = $form;
////        if ($this->getRequest()->isPost()) {
////            $formData = $this->getRequest()->getPost();
////            if ($form->isValid($formData)) {
////                $nombrecurso = $form->getValue('nombreCursos');
////                $cantidad = $form->getValue('cantidad');
////                $establecimiento = $form->getValue('idEstablecimiento');
...
////                    $curso->agregar($nombrecurso, $cantidad, $nivel, $establecimiento);
////
////                    //indico que despues de haber agregado el curso,
////                    //me redirija a la accion index de CursosController, es decir,
////                    //a la pagina que me muestra el listado de Cursos
////                    $db->commit();
////                    $this->_helper->redirector('index');
////                } catch (Exception $e) {
////                    // Si hubo problemas. Enviamos todo marcha atras
...
//        $tabla = new Application_Model_DbTable_Cursos();
...
//        }
    ...
    public function asignaturasAction()
    {
        ...
        if ($rol == '1' || $rol == '3' || $rol == '6') {
            ...
            $datoscurso = $tablacurso->getnombreactual($idcurso);
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            if ($rol == 1 || $rol == 3) {
                ...
                if (empty($data)) {
                    ...
                    //Iniciamos la transaccion de los datos
                    ...
                    try {
                        ...
                        for ($i = 0; $i < count($data); $i++) {
                            ...
                            $verifica = $asignaturascurso->validar($data[$i]['asignatura'], $idperiodo, $idcurso);
                            if ($verifica) {
                                $ultimoorden[0][max]++;
                                $asignaturascurso->agregar($data[$i]['asignatura'], 1, $ultimoorden[0]['max'], 1, $data[$i]['nucleo'], $idperiodo, $idcurso);
    ...
    public function guardaasignaturaAction()
    {
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            try {
                for ($i = 0; $i < count($data); $i++) {
                    $tabla->cambiarestadoorden($data[$i]['idasignatura'], $i + 1,$data[$i]['prioritaria'],$data[$i]['horas']);
            ...
            if ($this->getRequest()->isPost()) {
                ...
                if ($form->isValid($formData)) {
                    ...
                    try {
                        ...
                        $ultimoid = $asignaturas->ultimoid();
                        $asignaturascurso->agregar('' . $ultimoid[0]['max'] . '', $tipoasignatura, $orden, $promedio, $nucleo, $idperiodo, $curso, 0, 0);
...
//                    if ($fasignaturas[0]['tipoAsignatura'] == '4') {
//                        $form->tipoAsignatura->clearMultiOptions();
//                        $form->tipoAsignatura->addMultiOption("4", "Pertenece a una Asignatura Combinada");
//                        $form->tipoAsignatura->setValue(4);
//                    }
...
//                }
    ...
    public function configuraciontallerAction()
    {
        ...
        $this->view->headTitle($this->view->title);
        //creo objeto tabla Asignaturacursos()
        $asignaturacurso = new Application_Model_DbTable_Asignaturascursos();

        $idasig = $this->_getParam('id', 0);
        $idcurso = $this->_getParam('c', 0);
        $listalumnostaller = $asignaturacurso->getalumnostaller($idasig);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($formData)) {
                ...
                // Iniciamos la transaccion
                $db->beginTransaction();
                //Si la configuracion es para el todo el curso
                if ($tipo == 1) {
                    $listalumnos = '';

                } else { // si es por alumno
                ...
                try {
                    ...
                    $asignaturacurso->agregarconfiguracion($forma, $porcentaje, $idasignaturadestino, $idasignaturacurso, $segmento, $opcionsegmento, $tipo);
            ...
            $idcurso = $this->_getParam('c', 0);
            //Zend_Debug::dump($idasignaturataller);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($formData)) {
                ...
                try {
                    ...
                    //Cambiamos el estado de la asignatura que elimina de la lista
                    foreach ($resultado as $key => $value) {
                        $asignaturacurso->cambiartipoasignatura($value, 1, 1);
                    }
                    //Cambiamos el estado de la asignatura que agrega a la lista
    ...
    public function eliminarasignaturaAction()
    {
        ...
        //Obtenemos desde la tabla notas si existe alguna nota con la asignatura, curso y periodo
    ...
    public function listaalumnosAction()
    {
        ...
        if ($id > 0) {
            ...
            $listaalumnos = $modeloalumnos->getalumnostaller($id);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($formData)) {
                ...
                try {
                    ...
                    //$validarconcepto = $modeloasignatura->validarconceptoparvularia($idcurso, strtoupper($concepto[0]));
                    //if ($validarconcepto) {
    ...
    public function nuevosindicadoresAction()
    {
        ...
        if ($idcurso > 0) {
            ...
            //verificamos que el curso sea de Prebasica
            ...
            if ($datoscurso[0]['idCodigoTipo'] == 10) {
                ...
                if (count($listaasignatura) > 0) {
                    ...
                    //Creamos el array con los nuevos indicadores
                    $csvs = getcwd();
                    $csvs .= "/application/documentos/nuevos.xlsx";


                    require 'PHPExcel-1.8/Classes/PHPExcel.php';
                    //$tmpfname = APPLICATION_UPLOADS_DIR . "/" . $csvs;
                ...
                if($datoscurso[0]['idCodigoTipo']!=10){
                    ...
                    try {
                        for ($i=0;$i<count($nombres);$i++){
                            ...
                            $ultimoid = $asignaturas->ultimoid();
                            $asignaturascurso->agregar('' . $ultimoid[0]['max'] . '', $tipoasignatura, ($i+1), $incide, $nucleo, $idperiodo, $idcurso, 0, 0);
    ...
    public function horarioAction()
    {
        ...
        if ($idcurso > 0 && $idtipo>0) {
            ...
            //Obtenemos los datos del establecimiento por el id del curso
            ...
            //Validamos que los docentes no tengan tope en el horario
            for ($i = 0; $i < count($datosbloque); $i++) { //bloque


                for ($j = 1; $j < 6; $j++) {//Dias
                    //Checkeamos los si poseen datos
                    ...
                    if ($enviar) {
                        if (empty($bloque) || empty($dia)) {
                            ...
                            try {
                                ...
                                //Si posee un idHorario se actualiza
                                if ($idhorario > 0) {
                                    $horario = $modelocurso->listarbloqueid($idhorario);
                                    if (count($horario) > 0) {
                                        //Si la asignatura es la misma se mantienen las horas y se actualiza el registro
                                        if ($horario[0]['idAsignatura'] == $idasignatura) {
                                            ...
                                            //Se valida que la nueva asignatura posea horas disponibles para asignar
                                            $datosasignatura = $modeloasignatura->getdestino($idasignatura);
                                            if ($datosasignatura[0]['horas'] > $datosasignatura[0]['horasAsignadas']) {
                                                //si tiene disponible se actualiza el registro, se agrega la nueva hora a la asignatura y se resta la antigua

                                                //Restamos la hora a la asignatura antigua
                                                ...
                                                //Actualizamos el nuevo registro y agreamos la nueva hora asignada

                                                $modelocurso->actualizarbloque($idhorario, $idasignatura, $idcuenta);
                                                //Actualizamos la nueva hora asignada
                                    ...
                                    //Comprobamos que las horas asignadas no superen las establecidas
                                    $datosasignatura = $modeloasignatura->getdestino($idasignatura);
                                    if ($datosasignatura[0]['horas'] > $datosasignatura[0]['horasAsignadas']) {

                                        //Agregamos el bloque
                                        //Validamos si el bloque existe para el docente
                        ...
                        try {
                            ...
                            //Validamos que exista el registro que se elimina
                            $horario = $modelocurso->listarbloqueid($idhorario);
                            if (count($horario) > 0) {

                                $hora = $horario[0]['horasAsignadas'] - 1;
                                if ($hora >= 0) {
                                    //Actualizamos la hora asignada de la asignatura
